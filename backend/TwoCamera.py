...
def plot_box(labels, cords, frame):
    ...
    for i in range(n):
        row = cords[i]
        if (
            row[4] >= confidenceThreshold and int(labels[i]) == 0
        ):  # confidence is row[4]
            x1, y1, x2, y2 = (
                int(row[0] * x_shape),
                int(row[1] * y_shape),
            ...
            detections.append(
                ([x1, y1, int(x2 - x1), int(y2 - y1)], row[4].item(), "person")
            )
...
def checkCrossDoor(track_id, center, coords, f, left, ids_in, ids_out):
    ...
    if center[0] < leftDoorThreshold and left == True:
        ...
        for id in leftDoors.keys():
            ...
            slope = (pt0[0] - pt1[0]) / (
                pt0[1] - pt1[1]
            )  # slope from top point to bottom point
            assert slope < 0
            ...
            if (
                pt1[1] <= yb and yb <= pt0[1]
            ):  # bottom y coordinate in range of door y coords, pt1 is lower y val bound, pt0 is higher y val bound
        ...
        for id in rightDoors.keys():
            ...
            assert dxdy > 0
            if (
                pt1[1] <= yb and yb <= pt0[1] + 5
            ):  # bottom y coordinate in range of door y coords
                # print("bottom match")
            ...
            if 1:  # live feed code
                ...
                for i in range(len(frames)):
                    ...
                    if i == 1:
                        ...
                        subFrame = cv2.rotate(
                            subFrame, cv2.ROTATE_90_COUNTERCLOCKWISE
                        )  # for video in incorrect dimension
                    subFrame = cv2.resize(subFrame, (framex, framey))
                    processFeed(subFrame, outputFrames, outfile, i)
                    threading.Thread(
                        target=processFeed, args=(subFrame, outputFrames, outfile, i)
                    )
